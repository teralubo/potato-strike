...
function drawPotatoWallColumn(x, y, colW, wallH, hit, shade, distance, column, side = "") {
  ...
  ctx.strokeStyle = `rgba(0,0,0,${clamp(0.24 + distance / 4200, 0.2, 0.48)})`;
  ctx.lineWidth = 1;
  ctx.strokeRect(x + 0.5, y + 0.5, colW, wallH);
  ctx.fillStyle = `rgba(255,255,255,${clamp(0.11 - distance / 7600, 0.02, 0.09)})`;
  ctx.fillRect(x, y, colW + 1, Math.max(1, wallH * 0.025));
  ctx.fillStyle = `rgba(0,0,0,${clamp(0.2 + distance / 5200, 0.18, 0.46)})`;
  ctx.fillRect(x, y + wallH - Math.max(2, wallH * 0.04), colW + 1, Math.max(2, wallH * 0.04));
...
function render3d() {
  ...
  for (let i = 0; i < cols; i += 1) {
    ...
    drawPotatoWallColumn(x, y, colW, wallH, hitInfo.hit, shade, d, i, hitInfo.side);
